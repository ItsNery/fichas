...
function initCockpitForm() {
    ...
    if (form) {
        form.addEventListener("submit", function (e) {
            ...
            if (muni1 && muni2) {
                window.location.href = `/ficha-municipal/comparar/${muni1}/${muni2}`;
            }
        });
    }
}
...
function setupLazyCharts() {
    ...
    const combinedObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (itemData) {
                    renderCombinedChart(chartElement, itemData);
                    chartElement.style.opacity = "1";

                    const skeleton = document.getElementById("skeleton-combined-" + chartId);
                    if (skeleton) {
    ...
    const sepObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const chartElement = entry.target;
                const chartId = chartElement.getAttribute('data-chart-id');
                ...
                if (itemData) {
                    renderSeparateChart(chartElement, itemData, muniNum);
                    chartElement.style.opacity = "1";

                    const skeleton = document.getElementById(`skeleton-sep${muniNum}-${chartId}`);
                    if (skeleton) {
                        skeleton.style.transition = "opacity 0.3s ease";
                        skeleton.style.opacity = "0";
                        setTimeout(() => skeleton.remove(), 300);
    ...
    const sparkObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (itemData) {
                    const trendData = muniNum === "1" ? itemData.datos1.tendencia : itemData.datos2.tendencia;
                    if (trendData) {
...
function findChartDataById(id) {
    if (!window.ComparadorConfig || !window.ComparadorConfig.comparativaData) return null;
...
function renderCombinedChart(chartDom, itemData) {
    ...
    var option = {
        ...
        xAxis: {
            ...
            axisLabel: {
                rotate: echartsData.eje_x.categorias && echartsData.eje_x.categorias.length > 5 ? 30 : 0
            }
        },
        yAxis: { type: "value" },
        series: (echartsData.series || []).map((s) => {
            s.type = chartType;
            if (chartType === "line" || itemData.config.tipo_visualizacion.toLowerCase().includes("area")) {
                s.smooth = true;
                s.symbol = "circle";
                s.symbolSize = 8;
                if (itemData.config.tipo_visualizacion.toLowerCase().includes("area")) {
    ...
    if (itemData.config.ajustes_visuales) {
        try {
            var extra = typeof itemData.config.ajustes_visuales === "string"
...
function renderSeparateChart(chartDom, itemData, muniNum) {
            ...
            series: [
                {
                    ...
                    itemStyle: { color: muniNum === "1" ? "#861e34" : "#495057" },
                },
                {
                    name: "Mujeres",
                    type: "bar",
    ...
    if (!echartsData) {
        ...
        echartsData = {
            ...
            series: [{ name: muniNum === "1" ? window.ComparadorConfig.municipio1.nombre : window.ComparadorConfig.municipio2.nombre, type: 'bar', data: vars.map(v => v.valor) }]
        };
    }
    ...
    if (chartType === 'map') {
        var currentMunName = muniNum === "1" ? window.ComparadorConfig.municipio1.nombre.trim() : window.ComparadorConfig.municipio2.nombre.trim();
        ...
        option = {
            ...
            visualMap: {
                ...
                text: ["Alto", "Bajo"],
                calculable: true,
                inRange: { color: muniNum === "1" ? ["#fdf2f2", "#861e34"] : ["#f8f9fa", "#6c757d"] },
                bottom: 10,
                left: 10,
                itemHeight: 80
            },
        ...
        option = {
            ...
            color: muniNum === "1"
                ? ["#861e34", "#9b2a41", "#b1374e", "#c7455d", "#dd536c", "#eb6981"]
                : ["#6c757d", "#7b848c", "#8b949c", "#9ba4ad", "#abb5be", "#bcc6ce"],
            series: (echartsData.series || []).map((s) => {
                s.type = "treemap";
                s.roam = false;
                s.nodeClick = false;
        ...
        var isAxis = ["bar", "line", "area"].includes(chartType) || (echartsData.eje_x && echartsData.eje_x.categorias);
        option = {
            tooltip: {
        ...
        if (isAxis) {
            option.xAxis = {
                ...
                data: echartsData.eje_x && echartsData.eje_x.categorias ? echartsData.eje_x.categorias : [],
                axisLabel: {
                    rotate: echartsData.eje_x && echartsData.eje_x.categorias && echartsData.eje_x.categorias.length > 5 ? 30 : 0
                }
            };
...
function renderSparkline(sparkDom, trendData, muniNum) {
    ...
    var areaColorStart = muniNum === "1" ? "rgba(134, 30, 52, 0.3)" : "rgba(108, 117, 125, 0.3)";
    var areaColorEnd = muniNum === "1" ? "rgba(134, 30, 52, 0)" : "rgba(108, 117, 125, 0)";

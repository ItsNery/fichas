        ...
        <div class="row align-items-center">
            <div class="col-lg-7 text-white">
                ...
                <div class="d-flex align-items-center gap-3 mb-3">
                    ...
                    <button type="button" class="btn btn-outline-light btn-sm fw-bold px-3 py-1 rounded-pill" data-bs-toggle="modal" data-bs-target="#compararModal">
                        Comparar
                    </button>
                </div>
        ...
        <div class="row text-center justify-content-center flex-wrap g-4">
            <div class="col-6 col-md-2 col-lg flex-fill barra-indicadores__item">
                ...
                <div class="d-flex align-items-center justify-content-center gap-1">
                    ...
                    <button type="button" class="btn btn-link btn-sm p-0 text-gold similarity-popover-trigger hero-similarity-btn"
                        data-municipio-id="{{ $municipio->id }}"
                        data-config-id="poblacion"
                ...
                <div class="perfil-tarjeta">
                    <div class="perfil-tarjeta__body">
                        ...
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <h3 class="perfil-tarjeta__titulo">
                                ...
                                <span class="badge bg-secondary-subtle text-secondary border border-secondary-subtle px-2 py-1 ms-2"
                        ...
                        <div class="perfil-tarjeta__kpi-wrapper">
                                ...
                                @if($promMacro !== null && $promMacro > 0)
                                    ...
                                    <div class="perfil-tarjeta__benchmark-item">
                                        ...
                                        <span class="perfil-tarjeta__benchmark-indicator {{ $macroTrendClass }}">
                                            <i class="fa-solid {{ $macroIcon }}"></i> {{ number_format(abs($pctMacro), 1) }}%
                                        </span>
                                    </div>
                                @endif
                            ...
                            <div class="perfil-tarjeta__sparkline" id="sparkline-{{ $item['config']->id }}" data-chart-id="{{ $item['config']->id }}"></div>
                            @endif
                        </div>
                        @else
                        <div class="perfil-tarjeta__chart-wrapper {{ $gridClass == 'col-12' ? 'perfil-tarjeta__chart-wrapper--full' : 'perfil-tarjeta__chart-wrapper--half' }}">
                            ...
                            <div class="perfil-tarjeta__chart-box lazy-chart {{ $gridClass == 'col-12' ? 'perfil-tarjeta__chart-box--full' : 'perfil-tarjeta__chart-box--half' }}" id="chart-{{ $item['config']->id }}" data-chart-id="{{ $item['config']->id }}"></div>
                        </div>
                        @endif
                    </div>
    ...
    <div class="container">
        <div class="row align-items-center mb-4">
            <div class="col-md-8 text-start">
                ...
                <p class="text-muted mb-0">Identifica territorios con características demográficas o geográficas afines en el Estado de Puebla.</p>
            </div>
        </div>
                    ...
                    @else
                        <div class="d-flex flex-column gap-3">
                            @foreach($similaresPoblacion as $sim)
                                <div class="similar-card p-3 rounded border transition-all d-flex justify-content-between align-items-center">
                                    <div>
                                        <h4 class="h6 fw-bold text-vino mb-1">{{ $sim->nombre }}</h4>
                                        <p class="text-muted small mb-0">Población: <strong>{{ number_format($sim->poblacion_valor) }} hab.</strong></p>
                                    </div>
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('ficha-municipal.perfil', $sim->slug) }}" class="btn btn-outline-secondary btn-sm px-3 rounded-pill">Ver Perfil</a>
                                        <a href="{{ route('ficha-municipal.comparar', [$municipio->slug, $sim->slug]) }}" class="btn btn-vino btn-sm px-3 text-white rounded-pill">
                        ...
                        <div class="d-flex flex-column gap-3">
                            @foreach($similaresRegion as $sim)
                                <div class="similar-card p-3 rounded border transition-all d-flex justify-content-between align-items-center">
                                    <div>
                                        <h4 class="h6 fw-bold text-vino mb-1">{{ $sim->nombre }}</h4>
                                        <p class="text-muted small mb-0">Región: <strong>{{ $sim->microrregion->nombre ?? 'N/D' }}</strong></p>
                                    </div>
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('ficha-municipal.perfil', $sim->slug) }}" class="btn btn-outline-secondary btn-sm px-3 rounded-pill">Ver Perfil</a>
                                        <a href="{{ route('ficha-municipal.comparar', [$municipio->slug, $sim->slug]) }}" class="btn btn-vino btn-sm px-3 text-white rounded-pill">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <div class="modal-body p-4">
                <p class="text-muted small">Selecciona un municipio de la lista para contrastar sus indicadores demográficos, económicos y de gobierno lado a lado.</p>
                <form id="compare-modal-form" class="mt-3">
                    <div class="mb-4">
                        <label for="modal-select-muni" class="form-label fw-bold small text-uppercase">Municipio a comparar</label>
                    ...
                    <div class="d-grid">
                        <button type="submit" class="btn btn-vino py-2 fw-bold text-uppercase">Iniciar Comparativa</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (form) {
            form.addEventListener("submit", function(e) {
                ...
                if (selectedMuni) {
                    window.location.href = `/ficha-municipal/comparar/{{ $municipio->slug }}/${selectedMuni}`;
                }
            });
        }
    });
